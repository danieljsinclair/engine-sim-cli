...
// Report a --duration value that failed plain-seconds conversion and fail the
// run. Shared failure path for both catch clauses in resolvePlainDuration
// (S2738: specific first, catch-all last — one user-facing message either way).
bool durationConversionFailed(const std::string& raw) {
    std::cerr << "Could not convert: --duration = " << raw << "\n"
              << "Run with --help for more information.\n";
    return false;
}

// Plain-run --duration conversion: the raw string is plain seconds (CLI11
// cannot parse "1:30" into a double), converted so the interactive-default
// check and downstream wiring see the real number. The telemetry-driven
// window is parsed later (resolveTelemetryDurationWindow; it resolves onto
// --end-at). Returns false (after reporting) when the value is not plain
// seconds.
bool resolvePlainDuration(CommandLineArgs& args, bool telemetryDriven) {
    ...
    try {
        ...
        // std::stod reports every conversion failure (invalid_argument,
        // out_of_range, and the trailing-chars throw above) via std::exception.
        return durationConversionFailed(args.replay.durationRaw);
    } catch (...) {
        // Any other thrower during conversion is equally fatal to the run —
        // same user-facing failure path (S2738: rationale inside the braces).
        return durationConversionFailed(args.replay.durationRaw);
    }
    return true;
}
...
void resolveInteractiveDefault(CommandLineArgs& args, bool telemetryDriven) {
...
void warnIfPinTauOutsideStableWindow(const CommandLineArgs& args) {
...
// Auto-generate diagnostics filenames when a flag was given without a value.
// Gearbox log: local time (the gearbox log is a local diagnostic, not a
// traveling capture). Road-test CSV: UTC so a log's name is
// timezone-independent (captures travel across zones and the owner compares
// logs from different locations). Reruns never overwrite: every invocation
// gets a fresh timestamp.
void autoGenerateDiagnosticsFilenames(CommandLineArgs& args) {
...
// Flag-level implicits that need no validation: audio mode (sync-pull vs
// threaded), dyno percent -> fraction conversion, --silent muting, headless
// determinism, connect-demo defaults, and the interactive-explicit echo.
void applyModeImplicits(CommandLineArgs& args, double loadArg, bool threadedFlag,
                        bool silentFlag, bool interactiveExplicit) {
    args.syncPull = !threadedFlag && !args.deterministic;
    if (loadArg >= 0.0) {
        args.targetLoad = loadArg / 100.0;
    }
    if (silentFlag) {
        args.playAudio = true;
...
// --start-from / --end-at: parse time strings (plain seconds or mm:ss or
// hh:mm:ss) into doubles. Returns false (after reporting) on a bad value.
bool resolveReplayTimeBounds(CommandLineArgs& args) {
...
// Telemetry-driven --duration is a WINDOW: N seconds from the start point —
// replay measures from --start-from's arrival on the recording clock, live
// from attach (both providers run the same elapsed-seconds clock, so one
// formula covers both). Resolved onto the --end-at path so the provider's
// single time-slicing mechanism bounds the run; the raw --duration is
// consumed (reset to 0) so no downstream duration logic double-bounds it.
// (Supersedes the parse-time fail-fast that rejected every
// --duration + telemetry combination.)
// The raw string routes through the same parser as --start-from/--end-at
// (plain seconds, mm:ss and hh:mm:ss all work). Returns false (after
// reporting) on a bad value or a --duration/--end-at clash. Non-telemetry
// runs keep their already-converted args.duration (resolvePlainDuration) —
// only telemetry-driven runs consume the raw value onto the window.
bool resolveTelemetryDurationWindow(CommandLineArgs& args, bool telemetryDriven) {
    if (args.replay.durationRaw.empty() || !telemetryDriven) {
        return true;
    }
...
bool processArgs(CommandLineArgs& args, const std::string& scriptPath, const std::string& positionalEngineConfig, double loadArg, bool threadedFlag, bool silentFlag, bool interactiveExplicit) {
    applyModeImplicits(args, loadArg, threadedFlag, silentFlag, interactiveExplicit);

    const bool telemetryDriven = args.twin.liveTelemetry || !args.replay.telemetryPath.empty();
    if (!resolvePlainDuration(args, telemetryDriven)) {
        return false;
    }
    resolveInteractiveDefault(args, telemetryDriven);

    resolveReplayGearboxDefault(args);
    warnIfPinTauOutsideStableWindow(args);
    autoGenerateDiagnosticsFilenames(args);
    ...
    if (!resolveReplayTimeBounds(args)) {
        return false;
    }

    return resolveTelemetryDurationWindow(args, telemetryDriven);
}
