...
public:
    ...
    bool initialize(int sr) {
        ...
        context->circularBufferSize = 176400;  // 4 seconds at 44.1kHz (increased from 96000 to absorb timing jitter)
    ...
    void waitForCompletion() {
        // In streaming mode, AudioUnit handles playback asynchronously
        // No wait needed - cleanup is safe as AudioUnit will drain naturally
    }
...
int runSimulation(const CommandLineArgs& args) {
    ...
    if (args.sineMode) {
        ...
        if (args.playAudio) {
            ...
            const int preFillIterations = 240;  // ~4.0s at 60Hz - increased buffer for smoother audio (was 120 iterations ~2.0s)
        ...
        while ((!args.interactive && currentTime < args.duration) || (args.interactive && g_running.load())) {
            ...
            // CRITICAL FIX: Use IDENTICAL audio path as engine mode for DRY compliance
            // MockSynthesizer generates sine waves internally via writeToSynthesizer()
            // We read from it exactly like engine mode uses ReadAudioBuffer()
            // This ensures both modes use the same threading/synchronization model
            if (audioPlayer && audioPlayer->getContext()) {
                std::vector<float> tempBuffer(framesPerUpdate * 2);
                int samplesRead = 0;
                int readThisTime = 0;

                // Read from synthesizer - read whatever is available without retry
                // This prevents the main loop from blocking and falling behind
                EngineSimResult readResult = g_engineAPI.ReadAudioBuffer(handle,
                    tempBuffer.data(), framesPerUpdate, &readThisTime);
                if (readResult == ESIM_SUCCESS && readThisTime > 0) {
                    samplesRead = readThisTime;
                }

                if (samplesRead > 0) {
                    // Write to circular buffer (EXACT same path as engine audio)
                    audioPlayer->addToCircularBuffer(tempBuffer.data(), samplesRead);
                }
            }
    ...
    if (args.playAudio) {
        ...
        if (!audioPlayer->initialize(sampleRate)) {
            ...
            // Use increased buffering (4.0s) to absorb timing jitter while maintaining responsiveness
            const int preFillFrames = (sampleRate * 240) / 60;  // 240 iterations at 60Hz (~4.0s = ~176400 frames) for smooth audio
            std::vector<float> silenceBuffer(framesPerUpdate * 2, 0.0f);
        ...
        if (args.playAudio && audioPlayer) {
            ...
            for (int retry = 0; retry <= 3 && warmupRead < framesPerUpdate; retry++) {
                ...
                if (warmupRead < framesPerUpdate && retry < 3) {
                    std::this_thread::yield();  // Use yield instead of sleep
                }
            }
            if (warmupRead > 0) {
            ...
            if (args.playAudio && audioPlayer && audioPlayer->getContext()) {
                ...
                for (int retry = 0; retry <= maxRetries && totalRead < framesPerUpdate; retry++) {
                    ...
                    if (totalRead < framesPerUpdate && retry < maxRetries) {
                        // Brief yield to let audio thread process (use yield instead of sleep)
