// SyncPullRenderer.cpp - Sync-pull renderer implementation (consolidated)
// Combines lifecycle and rendering for lock-step mode
...
SyncPullRenderer::SyncPullRenderer(ILogging* logger)
    : defaultLogger_(logger ? nullptr : new ConsoleLogger())
    , logger_(logger ? logger : defaultLogger_.get()) {
}

void SyncPullRenderer::configure(const SimulationConfig& config) {
    preFillMs_ = config.preFillMs;
}

// === Lifecycle methods (from SyncPullAudioMode) ===

std::string SyncPullRenderer::getModeName() const {
    return "SyncPull";
}

void SyncPullRenderer::updateSimulation(EngineSimHandle handle, const EngineSimAPI& api,
...
void SyncPullRenderer::generateAudio(IAudioSource& audioSource, AudioPlayer* audioPlayer) {
    // Sync-pull mode: audio is generated by the callback, not here
    (void)audioSource;
    (void)audioPlayer;
}

std::string SyncPullRenderer::getModeString() const {
    return "sync-pull";
}

bool SyncPullRenderer::startAudioThread(EngineSimHandle handle, const EngineSimAPI& api,
                                       AudioPlayer* audioPlayer) {
...
void SyncPullRenderer::prepareBuffer(AudioPlayer* audioPlayer) {
...
void SyncPullRenderer::resetBufferAfterWarmup(AudioPlayer* audioPlayer) {
    // Sync-pull mode: no buffer reset needed
    (void)audioPlayer;
}

void SyncPullRenderer::startPlayback(AudioPlayer* audioPlayer) {
    if (audioPlayer) {
...
bool SyncPullRenderer::shouldDrainDuringWarmup() const {
    // Sync-pull mode: no draining during warmup
    return false;
}

std::unique_ptr<AudioUnitContext> SyncPullRenderer::createContext(
    int sampleRate,
...
// === Rendering methods (from SyncPullRenderer) ===
